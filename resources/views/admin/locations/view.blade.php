...
@section('title')
    @lang('admin/locations.breadcrumb_view', ['short' => $location->short])
@endsection
...
@section('content-header')
    ...
    <nav class="flex items-center gap-1 text-sm text-muted-foreground">
        ...
        <a href="{{ route('admin.locations') }}">@lang('admin/locations.breadcrumb_locations')</a>
        <x-icon name="chevron-right" class="size-3" />
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
        <div class="card">
            <header>
                <h3 class="text-lg font-semibold">@lang('admin/locations.location_details')</h3>
            </header>
            <section>
                <form action="{{ route('admin.locations.view', $location->id) }}" method="POST">
                    <div class="grid gap-6">
                        <div role="group" class="field">
                            <label for="pShort">@lang('admin/locations.short_code')</label>
                            <input type="text" id="pShort" name="short"  value="{{ $location->short }}" />
                        </div>
                        <div role="group" class="field">
                            <label for="pLong">@lang('admin/locations.description')</label>
            ...
            <footer>
                {!! csrf_field() !!}
                {!! method_field('PATCH') !!}
                <button name="action" value="edit" class="btn ml-auto" data-size="sm">@lang('admin/locations.save')</button>
    ...
    <div>
        <div class="card" data-variant="outline">
            <header>
                <h3 class="text-lg font-semibold">@lang('admin/locations.resource_allocation')</h3>
            </header>
            <section>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4>@lang('admin/locations.memory')</h4>
                        <div class="progress h-5">
                            <span style="width: {{ min($memoryPercent, 100) }}%; background-color: {{ $memoryColor }};"></span>
                        </div>
                        <p>
                            <strong>@lang('admin/locations.allocated'):</strong> {{ humanizeSize($allocatedMemory * 1024 * 1024) }}<br>
                            <strong>@lang('admin/locations.total'):</strong> {{ humanizeSize($totalMemory * 1024 * 1024) }}
                        </p>
                    </div>
                    <div>
                        <h4>@lang('admin/locations.disk')</h4>
                        <div class="progress h-5">
                            <span style="width: {{ min($diskPercent, 100) }}%; background-color: {{ $diskColor }};"></span>
                        </div>
                        <p>
                            <strong>@lang('admin/locations.allocated'):</strong> {{ humanizeSize($allocatedDisk * 1024 * 1024) }}<br>
                            <strong>@lang('admin/locations.total'):</strong> {{ humanizeSize($totalDisk * 1024 * 1024) }}
                        </p>
                    </div>
                </div>
            </section>
        ...
        <div class="card">
            <header>
                <h3 class="text-lg font-semibold">@lang('admin/locations.nodes')</h3>
            </header>
            <section>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('admin/locations.name')</th>
                                <th>@lang('admin/locations.fqdn')</th>
                                <th>@lang('admin/locations.memory')</th>
                                <th>@lang('admin/locations.disk')</th>
                                <th>@lang('admin/locations.servers')</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($location->nodes as $node)
