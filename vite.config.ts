...
try {
    branchName = child.execSync('git rev-parse --abbrev-ref HEAD', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().trimEnd();
    commitHash = child.execSync('git rev-parse HEAD', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().trimEnd();
} catch {
    branchName = 'unknown';
    commitHash = 'unknown';
}
